...
void TestUI::onStart() {
	...
	//加速エフェクト
	accEffectObj = new GameObject(getUser().getGameMediator());
	accEffectObj->getTransform().setLocalPosition(Vec3{ 0,0,1 });
	accEffectObj->getTransform().setLocalScale(Vec3{ 1280,720,1 });
	accSprite = accEffectObj->addComponent<GUISpriteRenderer>();
	accSprite->setTextureByName("title");
	accSprite->setColor(Color{ 1,0,0,1 });
	accSprite->setActive(false);
}
